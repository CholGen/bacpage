...
def load_input( directory: str, minimum_completeness: float ) -> dict[str, Path]:
    ...
    if common.is_project_directory( search_directory ):
        search_directory = search_directory / "results/consensus"
    fastas = common.find_files( directory=search_directory, extensions=[".fa", ".fasta"] )
...
def reconstruct_phylogeny( project_directory: str, configfile: str, minimum_completeness: float, threads: int,
    ...
    try:
        config = common.load_configfile( configfile, project_path )
    except Exception:
        print( "Error" )
        raise
    print( "Done" )
    ...
    # load snakefile
    snakefile = common.PACKAGE_DIR / "workflow/rules/phylogeny.smk"
    ...
    status = snakemake.snakemake(
        ...
        restart_times=common.RESTART_TIMES, config=config, cores=useable_threads, lock=False, quiet=not verbose
    )
    if not status:
